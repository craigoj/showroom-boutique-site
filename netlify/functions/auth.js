/*
  ...
  Create it under whichever account owns the repo at the time (craigoj until the
  transfer to Linda's own account happens, then recreate under hers):
    Homepage URL:               https://showroom-boutique-dayton.netlify.app (or the real domain once launched)
    Authorization callback URL: <same origin>/.netlify/functions/auth/callback

  Also requires the Netlify env var SITE_URL set on the production site (see
  vite.config.js) — base_url in public/admin/config.yml is substituted from it
  at build time, so without it the CMS auth popup targets the wrong origin even
  once the OAuth app exists.

  NOT YET CREATED as of 2026-07-25 — the GitHub OAuth app and Netlify env vars
  (OAUTH_CLIENT_ID / OAUTH_CLIENT_SECRET / SITE_URL) don't exist yet. Create the
  OAuth app and set all three env vars, then redeploy, to unblock the CMS's
  "Sign In with GitHub" path; until then this function 500s, which is expected
  ("Work with Local Repository" in the CMS works today without any of this).
